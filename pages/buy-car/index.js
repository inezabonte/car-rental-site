...
import { getSaleCar } from "lib/api";

export default function Buy({ fallback }) {
	const fetcher = async () => {
	...
	function useCars() {
		const { data, error } = useSWR("api/get-cars?q=buy", fetcher);
		return {
			cars: data,
...
export async function getStaticProps() {
	const cars = await getSaleCar();

	return {
		props: {
			fallback: {
				"api/get-cars?q=buy": cars,
			},
		},
		revalidate: 1,
	};
